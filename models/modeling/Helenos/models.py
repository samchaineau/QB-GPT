...
class StratEncoder(tf.keras.Model):
    ...
    def call(self, x):
        ...
        cross_att = self.CrossAttention(q = base_encoded, k = spe_encoded, v = spe_encoded)
        
        play_embed = self.Add([cross_att, base_encoded])
        
        spec_token = self.SpeToken(x)
        play_embed = self.Conc([spec_token, play_embed])
...
class StratFormer(tf.keras.Model):
    def __init__(self,
        ...
        self.Pred = tf.keras.layers.Dense(1, activation = "softmax")
